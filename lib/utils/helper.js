...
var serviceConfig = require('../src/testConfig.json');
 ...
 * @return res
 */
function restify (route, httpMethod, params, accessToken, headers) {
...
function Logger(){
    ...
    function log(logLevel, msg) {
        if(LOGGER === true) {
            logger.debug(new Date() + "%s %s", logLevel, msg);
        }
    }
}
...
function getSocketEndpoint(params) {
    ...
    logger.debug(params);
    var path = serviceConfig.socketEndpoint + "/?client_id=" + params.client_id + "&token=" + params.accessToken+ "&id=" + "120013" ;
    logger.debug(path);
    return path;
}

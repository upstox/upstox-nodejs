...
Ready-to-run Node.js examples for common Nifty 50 options strategies using the [Upstox Node.js SDK](https://www.npmjs.com/package/upstox-js-sdk).

Each script finds the required option legs via `InstrumentsApi.searchInstrument()` and places market orders via `OrderApiV3.placeOrder()`. Replace `ACCESS_TOKEN` with your token before running.

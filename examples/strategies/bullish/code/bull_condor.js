 ...
 * Buys ATM call, sells ATM+1 call, sells ATM+2 call, buys ATM+3 call for the next weekly expiry.
 */
...
instrumentsApi.searchInstrument('Nifty 50', {
    ...
    // Step 2: Find ATM+1 call (offset 1)
    instrumentsApi.searchInstrument('Nifty 50', {
        exchanges: 'NSE', segments: 'FO',
        ...
        // Step 3: Find ATM+2 call (offset 2)
        instrumentsApi.searchInstrument('Nifty 50', {
            exchanges: 'NSE', segments: 'FO',
            ...
            // Step 4: Find ATM+3 call (offset 3)
            instrumentsApi.searchInstrument('Nifty 50', {
                exchanges: 'NSE', segments: 'FO',
                instrumentTypes: 'CE', expiry: 'next_week', atmOffset: 3
            }, (error, data) => {
                if (error) { console.error('API error:', error); return; }
                ...
                orderApi.placeOrder(body1, {}, (error, result1) => {
                    if (error) { console.error('API error:', error); return; }
                    console.log('Leg 1 BUY ATM call order placed successfully. Order ID:', result1);

                    // Step 6: Sell ATM+1 call
                    ...
                    orderApi.placeOrder(body2, {}, (error, result2) => {
                        if (error) { console.error('API error:', error); return; }
                        console.log('Leg 2 SELL ATM+1 call order placed successfully. Order ID:', result2);

                        // Step 7: Sell ATM+2 call
                        ...
                        orderApi.placeOrder(body3, {}, (error, result3) => {
                            if (error) { console.error('API error:', error); return; }
                            console.log('Leg 3 SELL ATM+2 call order placed successfully. Order ID:', result3);

                            // Step 8: Buy ATM+3 call
                            ...
                            orderApi.placeOrder(body4, {}, (error, result4) => {
                                if (error) { console.error('API error:', error); return; }
                                console.log('Leg 4 BUY ATM+3 call order placed successfully. Order ID:', result4);
                            });
                        });
                    });
                });

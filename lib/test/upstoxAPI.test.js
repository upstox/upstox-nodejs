        ...
        it('Should get the live data for required exchange and instruments successfully', function (done) {
            ...
            var start = function () {
                ...
                upstox.getLiveFeeds(testdata.defaultLiveData).then(function (response) {
                    logger.debug("Live Data ----------- Response ----------" + JSON.stringify(response, null, 2));
                    expect(Object.keys(response.data)).to.have.length(2);
                    should.exist(response.data.timestamp);
                    should.exist(response.data.ltp);
                    done();
                }).catch(function(error){
                    console.log(error)
                    logger.error("Live Data ------------ Error ----------" + JSON.stringify(error, null, 2));
                    done();
                });
            }
        });
